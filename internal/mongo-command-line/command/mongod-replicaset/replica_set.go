...
func (p *prepareServer) Run() error {
	...
	case mongoreplicaset.AddOption:
		...
		//replSetColl := localDB.Collection("system.replset")
		//
		//// 检查是否只有一个文档
		...
		//// 获取现有配置
		//var c bson.M
		//err = replSetColl.FindOne(ctx, bson.D{}).Decode(&c)
		//if err != nil {
		//	return fmt.Errorf("no config object retrievable from local.system.replset: %w", err)
		//}
		//
		//// 增加版本号
		//c["version"] = c["version"].(int32) + 1
		//
		//// 找到现有成员的最大 _id
		//members := c["members"].(bson.A)
		//maxID := int32(0)
		//for _, member := range members {
		//	memberMap := member.(bson.M)
		//	if memberMap["_id"].(int32) > maxID {
		//		maxID = memberMap["_id"].(int32)
		//	}
		//}
		//
		//for _, mb := range p.server.Config.Members {
		//	bsonData := mb.BsonData()
		//	if bsonData == nil {
		//		return fmt.Errorf("member failed to bson marshal struct")
		//	}
		//
		...
		//	log.Debugf("max id %d : map %v", maxID, newMember)
		//}
		//
		//c["members"] = members
		//
		//command := bson.D{{"replSetReconfig", c}}
		//adminDB := client.Database("admin")
		//var result bson.M
		//err = adminDB.RunCommand(ctx, command).Decode(&result)
		//if err != nil {
		//	return fmt.Errorf("replSetReconfig command failed: %w", err)
		//}
		//
		//return nil
